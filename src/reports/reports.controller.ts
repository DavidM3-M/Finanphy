...
export class ReportsController {
  ...
  // src/reports/reports.controller.ts (fragmento)
@Get('monthly')
...
async getMonthlyReport(
  ...
  const headerCompanyId = (req.headers['x-company-id'] ?? req.headers['x-company-Id']) as string | undefined;
  const tokenCompanyId = currentUser?.companyId ?? currentUser?.company?.id ?? currentUser?.company_id;
  ...
  if (headerCompanyId && String(headerCompanyId).trim() !== '') {
    ...
    throw new BadRequestException('companyId faltante: pásalo en header X-Company-Id, en el token o como query (Admin)');
  }
  ...
  return this.reportsService.generateMonthlyReportForCompany(targetCompanyId, {
    period,
    topN,
